...
class CPUCoherenceBeamformer(Tabbed, Parallelized):
    nwrkr : int = 1
    ...
    def __beamform_single__(id, itx, irx):
        ...
        pttx = CPUCoherenceBeamformer.__offset_pnt__(params['pttx'], txoff)
        ptrx = CPUCoherenceBeamformer.__offset_pnt__(params['ptrx'], rxoff)
        patx = CPUCoherenceBeamformer.__offset_pnt__(params['patx'], txoff)
        parx = CPUCoherenceBeamformer.__offset_pnt__(params['parx'], rxoff)
        psig = CPUCoherenceBeamformer.__offset_pnt__(params['psig'], rfoff)
    ...
    def __call__(self, txrxt:ndarray):
        ...
        # delay and apodize
        iterator = product([self.id], range(self.ntx))
        if self.nwrkr > 1:
            raise Exception("Coherence beamformer has not been parallelized yet")
            #self.pool.starmap(CPUCoherenceBeamformer.__beamform_single__, iterator)
        else:
            rxtraces = []
            for irx in tqdm(range(self.nrx)):
